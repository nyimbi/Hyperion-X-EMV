...
- [x] Pre-lab APDU trace fixture (`prelab_apdu_trace_pack.jsonl`) – deterministic masked JSONL fixture generated by `cargo run --example krn_prelab_trace_pack`; covers trace-pack metadata, scenario expectations with FSM and APDU status actions, trace identity, PAN masking, Track 2 suppression, GENERATE AC cryptogram suppression, profile-defined issuer application data suppression, issuer-authentication/script status evidence, issuer-script retry status evidence, and APDU follow-up status evidence; full lab/test-tool trace pack remains pending
