...
- [x] Pre-lab APDU trace fixture (`prelab_apdu_trace_pack.jsonl`) – deterministic masked JSONL fixture generated by `cargo run --example krn_prelab_trace_pack`; covers trace-pack metadata, trace identity, PAN masking, GENERATE AC cryptogram suppression, issuer-authentication/script status evidence, and APDU follow-up status evidence; full lab/test-tool trace pack remains pending
