...
- [x] Reproducible build provenance – `cargo run --example krn_build_manifest -- src Cargo.lock Cargo.toml docs/...` emits canonical JSON with SHA-256 for every kernel source module, the manifest tool, profile dictionary generator, decoder utility, lockfile, and controlled annexes
