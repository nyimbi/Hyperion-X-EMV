...
1. Maintain and extend the `krn_emv_decode` example so lab-trace triage stays
   ...
   primitive tag-list, numeric-code, fixed numeric amount, EMV date, currency
   exponent, transaction type, terminal-type, AIP, Application Usage Control,
   CVM Results, bitmap, CID, GENERATE AC response, issuer host-response,
