...
use hyperion_emv::ffi::{
    ...
    krn_error_description, krn_error_name, krn_error_table_len, krn_get_callback_timeout_policy,
    krn_get_conformance_statement_json, krn_get_final_outcome, krn_get_fsm_state,
    krn_get_issuer_script_result, krn_get_issuer_script_result_count,
    krn_get_issuer_script_result_identifier, krn_get_issuer_script_result_phase,
    krn_get_issuer_script_result_position, krn_get_last_error, krn_get_online_authorization_data,
    krn_get_profile_sha256, krn_get_profile_version, krn_init, krn_load_profiles_verified,
    krn_mask_apdu_command_json, krn_mask_apdu_response_json, krn_process_final_generate_ac,
    ...
    krn_set_trm_random_selection_sample, KrnCallbackTimeoutPolicy, KrnConfigBlob, KrnOutcome,
    KrnRuntime, KrnTxnParams, KRN_ABI_VERSION, KRN_CALLBACK_TIMEOUT_MAX_MS,
    KRN_CALLBACK_TIMEOUT_MIN_MS, KRN_ISSUER_SCRIPT_IDENTIFIER_LEN,
...
const TOOLING_COMPLETENESS_AUDIT: &str = include_str!("../docs/tooling_completeness_audit.json");
const TOOLING_COMPLETENESS_AUDIT_MARKDOWN: &str =
    include_str!("../docs/tooling_completeness_audit.md");
const CERTIFICATION_EVIDENCE_CHECKLIST: &str =
    include_str!("../docs/certification_evidence_checklist.json");
const CERTIFICATION_EVIDENCE_MARKDOWN: &str =
    include_str!("../docs/certification_evidence_checklist.md");
const CERTIFICATION_EVIDENCE_INTAKE: &str =
    include_str!("../docs/certification_evidence_intake.json");
const CERTIFICATION_EVIDENCE_INTAKE_MARKDOWN: &str =
    include_str!("../docs/certification_evidence_intake.md");
const CERTIFICATION_ARTIFACT_IMPORT_PLAN: &str =
    include_str!("../docs/certification_artifact_import_plan.json");
const CERTIFICATION_ARTIFACT_IMPORT_PLAN_MARKDOWN: &str =
    include_str!("../docs/certification_artifact_import_plan.md");
const CERTIFICATION_FREEZE_MANIFEST: &str =
    include_str!("../docs/certification_freeze_manifest.json");
const CERTIFICATION_FREEZE_MANIFEST_MARKDOWN: &str =
    include_str!("../docs/certification_freeze_manifest.md");
const CERTIFICATION_SECURITY_ASSESSMENT_PLAN: &str =
    include_str!("../docs/certification_security_assessment_plan.json");
const CERTIFICATION_SECURITY_ASSESSMENT_PLAN_MARKDOWN: &str =
    include_str!("../docs/certification_security_assessment_plan.md");
const CERTIFICATION_DEVICE_EVIDENCE_PLAN: &str =
    include_str!("../docs/certification_device_evidence_plan.json");
const CERTIFICATION_DEVICE_EVIDENCE_PLAN_MARKDOWN: &str =
    include_str!("../docs/certification_device_evidence_plan.md");
const CERTIFICATION_INTEGRATION_REPORT_PLAN: &str =
    include_str!("../docs/certification_integration_report_plan.json");
const CERTIFICATION_INTEGRATION_REPORT_PLAN_MARKDOWN: &str =
    include_str!("../docs/certification_integration_report_plan.md");
const CERTIFICATION_REPORT_PACK: &str = include_str!("../docs/certification_report_pack.json");
const CERTIFICATION_REPORT_MARKDOWN: &str = include_str!("../docs/certification_report_pack.md");
const CERTIFICATION_REPORT_UI: &str = include_str!("../docs/certification_report_ui.html");
...
fn rtm_external_lab_gates_are_explicit() {
    ...
    assert!(LAB_SUBMISSION_MANIFEST
        .contains("APDU trace logs (masked) for all applicable lab/test-tool"));
...
fn rtm_promotes_gpo_and_read_record_evidence() {
    for csv in [CURRENT_RTM, LEGACY_RTM] {
        ...
        assert!(sfi.contains("accepts_maximum_afl_entry_count_without_overflow"));
...
fn lab_manifest_and_provenance_cover_reproducible_build_artifacts() {
    ...
    assert!(LAB_SUBMISSION_MANIFEST.contains("Certification evidence checklist"));
    assert!(LAB_SUBMISSION_MANIFEST.contains("certification_evidence_checklist.json"));
    assert!(LAB_SUBMISSION_MANIFEST.contains("krn_certification_evidence_checklist"));
    assert!(LAB_SUBMISSION_MANIFEST.contains("maps every `CERT-OPEN-*` blocker"));
    assert!(LAB_SUBMISSION_MANIFEST.contains("Certification evidence intake ledger"));
    assert!(LAB_SUBMISSION_MANIFEST.contains("certification_evidence_intake.json"));
    assert!(LAB_SUBMISSION_MANIFEST.contains("krn_certification_evidence_intake"));
    assert!(LAB_SUBMISSION_MANIFEST.contains("Certification artifact import adapters"));
    assert!(LAB_SUBMISSION_MANIFEST.contains("certification_artifact_import_plan.json"));
    assert!(LAB_SUBMISSION_MANIFEST.contains("krn_certification_artifact_import"));
    assert!(LAB_SUBMISSION_MANIFEST
        .contains("real lab, scheme, CAPK, vector, device, and report artifacts"));
    assert!(LAB_SUBMISSION_MANIFEST.contains("supersession history"));
    assert!(LAB_SUBMISSION_MANIFEST.contains("licensed/lab reconciliation is still required"));
    assert!(LAB_SUBMISSION_MANIFEST.contains("Certification freeze manifest"));
    assert!(LAB_SUBMISSION_MANIFEST.contains("certification_freeze_manifest.json"));
    assert!(LAB_SUBMISSION_MANIFEST.contains("krn_certification_freeze_manifest"));
    assert!(LAB_SUBMISSION_MANIFEST.contains("submitted-build binding template"));
    assert!(LAB_SUBMISSION_MANIFEST.contains("SHA-256 slots"));
    assert!(LAB_SUBMISSION_MANIFEST.contains("Certification security assessment plan"));
    assert!(LAB_SUBMISSION_MANIFEST.contains("certification_security_assessment_plan.json"));
    assert!(LAB_SUBMISSION_MANIFEST.contains("krn_certification_security_assessment_plan"));
    assert!(LAB_SUBMISSION_MANIFEST.contains("external-assessor control plan"));
    assert!(LAB_SUBMISSION_MANIFEST.contains("accepted third-party report pending"));
    assert!(LAB_SUBMISSION_MANIFEST.contains("Certification device evidence plan"));
    assert!(LAB_SUBMISSION_MANIFEST.contains("certification_device_evidence_plan.json"));
    assert!(LAB_SUBMISSION_MANIFEST.contains("krn_certification_device_evidence_plan"));
    assert!(LAB_SUBMISSION_MANIFEST.contains("device/L1/PED evidence plan"));
    assert!(LAB_SUBMISSION_MANIFEST.contains("Device and L1 evidence package"));
    assert!(LAB_SUBMISSION_MANIFEST.contains("Certification integration report plan"));
    assert!(LAB_SUBMISSION_MANIFEST.contains("certification_integration_report_plan.json"));
    assert!(LAB_SUBMISSION_MANIFEST.contains("krn_certification_integration_report_plan"));
    assert!(LAB_SUBMISSION_MANIFEST
        .contains("full integration-report and APDU trace-pack evidence plan"));
    assert!(LAB_SUBMISSION_MANIFEST.contains("Level 3/acquirer reconciliation"));
    ...
    assert!(LAB_SUBMISSION_MANIFEST.contains("certification_report_ui.html"));
    assert!(LAB_SUBMISSION_MANIFEST.contains("certification_report_pack.json"));
    ...
    assert!(LAB_SUBMISSION_MANIFEST.contains("Basic SoftPoS integration example"));
    assert!(LAB_SUBMISSION_MANIFEST.contains("krn_basic_softpos"));
    ...
    assert!(LAB_SUBMISSION_MANIFEST.contains("Certification attachment audit"));
    assert!(LAB_SUBMISSION_MANIFEST.contains("krn_certification_attachment_audit"));
    assert!(LAB_SUBMISSION_MANIFEST.contains("Coverage package audit"));
    ...
    let expected_build_manifest_command = "cargo run --quiet --example krn_build_manifest -- src Cargo.lock Cargo.toml .github/workflows/prelab.yml docs/spec.md docs/lab_submission_manifest.md docs/requirements_traceability.csv docs/requirements-traceability-matrix.csv docs/scheme_profiles.cert.json docs/scheme_profile_dictionary.md docs/oda_test_vectors.json docs/tlv_catalogue.csv docs/state_machine.csv docs/bitmap_catalogue.csv docs/performance_profile.csv docs/abi_conformance_statement.json docs/prelab_apdu_trace_pack.jsonl docs/prelab_trace_pack_audit.json docs/prelab_trace_pack_audit.md docs/prelab_quality_gates.json docs/prelab_no_crash_smoke.json docs/prelab_static_fuzz_plan.json docs/prelab_fuzz_seed_corpus.json docs/public_standards_watch.json docs/tooling_completeness_audit.json docs/tooling_completeness_audit.md docs/certification_evidence_checklist.json docs/certification_evidence_checklist.md docs/certification_evidence_intake.json docs/certification_evidence_intake.md docs/certification_artifact_import_plan.json docs/certification_artifact_import_plan.md docs/certification_freeze_manifest.json docs/certification_freeze_manifest.md docs/certification_security_assessment_plan.json docs/certification_security_assessment_plan.md docs/certification_device_evidence_plan.json docs/certification_device_evidence_plan.md docs/certification_integration_report_plan.json docs/certification_integration_report_plan.md docs/certification_report_pack.json docs/certification_report_pack.md docs/certification_report_ui.html docs/certification_open_issues.md docs/standards_watch.md docs/open_source.md docs/coverage.md scripts/coverage_100.sh examples/krn_build_manifest.rs examples/krn_abi_conformance_statement.rs examples/krn_cabi_script_adapter.rs examples/krn_certification_attachment_audit.rs examples/krn_coverage_package_audit.rs examples/krn_trace_pack_audit.rs examples/krn_certification_evidence_checklist.rs examples/krn_certification_evidence_intake.rs examples/krn_certification_artifact_import.rs examples/krn_certification_freeze_manifest.rs examples/krn_certification_security_assessment_plan.rs examples/krn_certification_device_evidence_plan.rs examples/krn_certification_integration_report_plan.rs examples/krn_certification_report_ui.rs examples/krn_certification_workspace.rs examples/krn_certification_wizard.rs examples/krn_tooling_completeness_audit.rs examples/krn_basic_pos.rs examples/krn_basic_softpos.rs examples/krn_callback_timeout_policy.rs examples/krn_variable_data_boundary_audit.rs examples/krn_scheme_profile_dictionary.rs examples/krn_prelab_trace_pack.rs examples/krn_prelab_quality_gates.rs examples/krn_prelab_no_crash_smoke.rs examples/krn_prelab_static_fuzz_plan.rs examples/krn_prelab_fuzz_seed_corpus.rs examples/krn_public_standards_watch.rs examples/krn_emv_decode.rs";
    ...
    let mut input_paths = vec![
        ".github/workflows/prelab.yml".to_string(),
        ...
        "docs/tooling_completeness_audit.json".to_string(),
        "docs/tooling_completeness_audit.md".to_string(),
        "docs/certification_evidence_checklist.json".to_string(),
        "docs/certification_evidence_checklist.md".to_string(),
        "docs/certification_evidence_intake.json".to_string(),
        "docs/certification_evidence_intake.md".to_string(),
        "docs/certification_artifact_import_plan.json".to_string(),
        "docs/certification_artifact_import_plan.md".to_string(),
        "docs/certification_freeze_manifest.json".to_string(),
        "docs/certification_freeze_manifest.md".to_string(),
        "docs/certification_security_assessment_plan.json".to_string(),
        "docs/certification_security_assessment_plan.md".to_string(),
        "docs/certification_device_evidence_plan.json".to_string(),
        "docs/certification_device_evidence_plan.md".to_string(),
        "docs/certification_integration_report_plan.json".to_string(),
        "docs/certification_integration_report_plan.md".to_string(),
        "docs/certification_report_pack.json".to_string(),
        "docs/certification_report_pack.md".to_string(),
        "docs/certification_report_ui.html".to_string(),
        ...
        "examples/krn_certification_artifact_import.rs".to_string(),
        "examples/krn_certification_freeze_manifest.rs".to_string(),
        "examples/krn_certification_security_assessment_plan.rs".to_string(),
        "examples/krn_certification_device_evidence_plan.rs".to_string(),
        "examples/krn_certification_integration_report_plan.rs".to_string(),
        "examples/krn_certification_report_ui.rs".to_string(),
        "examples/krn_certification_workspace.rs".to_string(),
        "examples/krn_tooling_completeness_audit.rs".to_string(),
        "examples/krn_basic_pos.rs".to_string(),
        "examples/krn_basic_softpos.rs".to_string(),
        "examples/krn_callback_timeout_policy.rs".to_string(),
        "examples/krn_variable_data_boundary_audit.rs".to_string(),
        ...
        "scripts/coverage_100.sh".to_string(),
    ];
    ...
    for required in [
        ...
        "examples/krn_certification_security_assessment_plan.rs",
        "examples/krn_certification_device_evidence_plan.rs",
        "examples/krn_certification_integration_report_plan.rs",
...
fn certification_open_issues_register_tracks_external_blockers() {
    ...
    assert!(CERTIFICATION_OPEN_ISSUES.contains("TRMD, C-4, and contact-feature bulletin set"));
    assert!(LAB_SUBMISSION_MANIFEST.contains("C-8 v1.1 / SB 325"));
    assert!(LAB_SUBMISSION_MANIFEST.contains("SB 326, SB 327, DSB 331"));
    assert!(LAB_SUBMISSION_MANIFEST.contains("SB 314 / DSB 324 / DSB 308"));
    ...
    assert!(STANDARDS_WATCH.contains("SB 314"));
    assert!(STANDARDS_WATCH.contains("Update to TRMD"));
    assert!(STANDARDS_WATCH.contains("DSB 324"));
    assert!(STANDARDS_WATCH.contains("Updates to C-4"));
...
#[test]
fn krn_cfg_005_variable_data_boundary_is_audited() {
    let mut source_paths = fs::read_dir("src")
        .unwrap()
        .map(|entry| entry.unwrap().path())
        .filter(|path| path.extension().is_some_and(|extension| extension == "rs"))
        .collect::<Vec<_>>();
    source_paths.sort();
    let owned_sources = source_paths
        .iter()
        .map(|path| {
            (
        ...
        .collect::<Vec<_>>();
    let inputs = owned_sources
        .iter()
        .map(|(path, contents)| BoundaryAuditInput { path, contents })
        .collect::<Vec<_>>();
    let audit = audit_variable_data_boundary(&inputs).unwrap();

    assert!(audit.passed(), "unexpected findings: {:?}", audit.findings);
    assert!(audit.canonical_json().contains("\"status\":\"pass\""));
    assert!(include_str!("../docs/spec.md").contains("KRN-CFG-005"));
    assert!(include_str!("../docs/spec.md").contains("pre-lab variable-data"));
    assert!(include_str!("../docs/spec.md").contains("boundary audit"));
    assert!(include_str!("../docs/eng_notes.md")
        .contains("Everything variable must live in a signed, versioned profile bundle"));
    for csv in [CURRENT_RTM, LEGACY_RTM] {
        let row = csv_row_for_requirement(csv, "KRN-CFG-005").expect("RTM row exists");
        assert!(row.contains("audit_passes_when_scheme_values_are_only_in_test_modules"));
        assert!(row.contains("audit_fails_when_production_source_contains_scheme_values"));
        assert!(row.contains("default_audit_over_src_passes_current_boundary"));
        assert!(row.contains("krn_cfg_005_variable_data_boundary_is_audited"));
    }
    assert!(README.contains("krn_variable_data_boundary_audit"));
    assert!(LAB_SUBMISSION_MANIFEST.contains("Variable-data boundary audit"));
    assert!(PRELAB_QUALITY_GATES.contains("PRELAB-VARIABLE-DATA-BOUNDARY"));
    assert!(PRELAB_QUALITY_GATES.contains("krn_variable_data_boundary_audit"));
    assert!(CERTIFICATION_REPORT_PACK.contains("VARIABLE-DATA-BOUNDARY"));
    assert!(CERTIFICATION_REPORT_PACK.contains("krn_variable_data_boundary_audit"));
    assert!(CERTIFICATION_REPORT_PACK.contains("TOOLING-COMPLETENESS"));
    assert!(CERTIFICATION_REPORT_PACK.contains("docs/tooling_completeness_audit.json"));
    assert!(CERTIFICATION_REPORT_PACK.contains("krn_tooling_completeness_audit"));
    assert!(PRELAB_CI_WORKFLOW.contains("Variable data boundary audit"));
    assert!(include_str!("../docs/tutorial/05-using-hyperion.md")
        .contains("krn_variable_data_boundary_audit"));
}
...
fn rtm_promotes_dda_internal_authenticate_evidence() {
    for csv in [CURRENT_RTM, LEGACY_RTM] {
        ...
        assert!(
            signed_dynamic_data.contains("rejects_empty_internal_authenticate_icc_dynamic_number")
        );
        assert!(
...
fn rtm_promotes_api_error_boundary_evidence() {
    for csv in [CURRENT_RTM, LEGACY_RTM] {
        ...
        assert!(callbacks
            .contains("krn_api_001_002_004_006_runtime_callbacks_are_versioned_and_bounded"));
        assert!(callbacks.contains("krn_api_006_exposes_callback_timeout_policy"));
...
fn krn_dda_002_oda_006_requires_signed_dynamic_application_data() {
    ...
    assert_eq!(
        parse_internal_authenticate_response(&hex("770E9F4B08A1A2A3A4A5A6A7A89F4C00")).unwrap_err(),
        hyperion_emv::KernelError::ParseError
    );
    assert_eq!(
...
#[test]
fn krn_api_006_exposes_callback_timeout_policy() {
    unsafe {
        assert_eq!(
        ...
        let mut policy = KrnCallbackTimeoutPolicy {
            abi_version: KRN_ABI_VERSION,
            struct_size: core::mem::size_of::<KrnCallbackTimeoutPolicy>() as u32,
            min_timeout_ms: 0,
            max_timeout_ms: 0,
...
fn krn_log_001_masks_sensitive_tlv_and_gac_trace_values() {
    ...
    let iad = mask_tlv_value(&[0x9f, 0x10], &hex("AABBCC"), LogPolicy::production());
    assert_eq!(
        iad.value,
...
fn prelab_apdu_trace_pack_is_replayable_masked_and_scoped() {
    ...
    .unwrap();
    let track2_record_script = ReplayScript::new(vec![track2_record]).unwrap();
    ...
    for (case_id, script) in [
        ...
        generated.push_str(match case_id {
            ...
            "prelab.masking.issuer-script-retry" => {
                "{\"type\":\"trace-scenario\",\"case_id\":\"prelab.masking.issuer-script-retry\",\"expected_step_count\":2,\"expected_fsm_events\":[\"ScriptRetryWithCorrectLe\",\"ScriptWarning\"],\"expected_fsm_actions\":[\"ProcessIssuerScripts\",\"RetryIssuerScriptWithCorrectLe\",\"CaptureScriptStatus\"],\"expected_status_actions\":[\"RetryWithCorrectLe6cxx\"],\"expected_command_flow\":[\"issuer-script-command\",\"issuer-script-command-retry\"],\"expected_response_shapes\":[\"status-only-retry-le\",\"status-only-warning\"],\"expected_terminal_outcome\":\"issuer-script-warning-recorded\",\"expected_tlv_stream_count\":0,\"masking_assertions\":[\"full-apdu-disabled\",\"issuer-script-command-data-suppressed\",\"issuer-script-retry-status-recorded\"]}\n"
            }
            "prelab.masking.track2-record" => {
                "{\"type\":\"trace-scenario\",\"case_id\":\"prelab.masking.track2-record\",\"expected_step_count\":1,\"expected_fsm_events\":[\"RecordRead\"],\"expected_fsm_actions\":[\"ReadRecords\"],\"expected_status_actions\":[],\"expected_command_flow\":[\"read-record\"],\"expected_response_shapes\":[\"record-template-70\"],\"expected_terminal_outcome\":\"record-data-collected\",\"expected_tlv_stream_count\":0,\"masking_assertions\":[\"full-apdu-disabled\",\"track2-suppressed\"]}\n"
            }
            "prelab.masking.follow-up-status" => {
                "{\"type\":\"trace-scenario\",\"case_id\":\"prelab.masking.follow-up-status\",\"expected_step_count\":4,\"expected_fsm_events\":[\"GpoTemplate77\",\"GacArqc\"],\"expected_fsm_actions\":[\"BuildGpo\",\"RequestFirstGenerateAc\",\"BuildHostRequest\"],\"expected_status_actions\":[\"GetResponse61xx\",\"RetryWithCorrectLe6cxx\"],\"expected_command_flow\":[\"get-processing-options\",\"get-response\",\"generate-ac\",\"generate-ac-retry\"],\"expected_response_shapes\":[\"status-only-get-response\",\"gpo-template-77\",\"status-only-retry-le\",\"gac-template-80\"],\"expected_terminal_outcome\":\"first-generate-ac-complete\",\"expected_tlv_stream_count\":0,\"masking_assertions\":[\"full-apdu-disabled\",\"follow-up-response-tag-masked\",\"transaction-cryptogram-suppressed\"]}\n"
            }
            "prelab.masking.generate-ac-status-only" => {
                "{\"type\":\"trace-scenario\",\"case_id\":\"prelab.masking.generate-ac-status-only\",\"expected_step_count\":1,\"expected_fsm_events\":[\"GacStatusFailure\"],\"expected_fsm_actions\":[\"RequestFirstGenerateAc\",\"FailClosed\"],\"expected_status_actions\":[\"FailCardRemoved\"],\"expected_command_flow\":[\"generate-ac\"],\"expected_response_shapes\":[\"status-only-failure\"],\"expected_terminal_outcome\":\"generate-ac-status-failure\",\"expected_tlv_stream_count\":0,\"masking_assertions\":[\"full-apdu-disabled\",\"status-only-response-recorded\",\"no-response-body-parsing\"]}\n"
            }
    ...
    assert!(
        PRELAB_APDU_TRACE_PACK.contains("\"expected_terminal_outcome\":\"record-data-collected\"")
    );
    ...
    assert!(PRELAB_APDU_TRACE_PACK
        .contains("\"expected_command_flow\":[\"select-aid\",\"read-record\",\"generate-ac\"]"));
    assert!(PRELAB_APDU_TRACE_PACK.contains("\"expected_command_flow\":[\"get-processing-options\",\"get-response\",\"generate-ac\",\"generate-ac-retry\"]"));
    assert!(PRELAB_APDU_TRACE_PACK
        .contains("\"expected_response_shapes\":[\"fci-template-6f\",\"record-template-70\",\"gac-template-77\"]"));
    assert!(PRELAB_APDU_TRACE_PACK.contains("\"expected_response_shapes\":[\"status-only-get-response\",\"gpo-template-77\",\"status-only-retry-le\",\"gac-template-80\"]"));
    assert!(
        PRELAB_APDU_TRACE_PACK.contains("\"expected_response_shapes\":[\"status-only-failure\"]")
    );
    ...
    assert!(PRELAB_APDU_TRACE_PACK.contains("\"tag\":\"57\""));
    assert!(PRELAB_APDU_TRACE_PACK.contains("\"reason\":\"track2\""));
...
fn prelab_trace_pack_audit_is_reproducible_masked_and_scoped() {
    ...
    assert!(README.contains("krn_tooling_completeness_audit"));
    assert!(README.contains("krn_certification_wizard"));
    assert!(TUTORIAL_DATA_BUNDLES.contains("krn_certification_wizard"));
    assert!(TOOLING_COMPLETENESS_AUDIT.contains("examples/krn_certification_wizard.rs"));
...
#[test]
fn certification_artifact_import_adapters_are_reproducible_and_fail_closed() {
    let json = certification_artifact_import_plan_json(KRN_ABI_VERSION);
    let markdown = certification_artifact_import_plan_markdown(KRN_ABI_VERSION);

    assert_eq!(CERTIFICATION_ARTIFACT_IMPORT_PLAN, json);
    assert_eq!(CERTIFICATION_ARTIFACT_IMPORT_PLAN_MARKDOWN, markdown);
    for required in [
        "LAB-APPROVAL",
        "SCHEME-PROFILE",
        "CAPK",
        ...
        "DEVICE",
        "REPORT",
        "private signing material is rejected",
        "external authorities still decide acceptance",
    ] {
        assert!(CERTIFICATION_ARTIFACT_IMPORT_PLAN.contains(required));
    }
    assert!(CERTIFICATION_ARTIFACT_IMPORT_PLAN_MARKDOWN
        .contains("Hyperion Certification Artifact Import Plan"));
    ...
    fs::write(root.join("capk/public.pem"), b"capk-public").unwrap();
    fs::write(root.join("capk/private.key"), b"secret").unwrap();
    let report = import_certification_artifacts(&root).unwrap();
    let report_json = certification_artifact_import_report_json(KRN_ABI_VERSION, &report);
...
#[test]
fn tooling_completeness_audit_is_reproducible_and_preserves_external_gates() {
    let json = tooling_completeness_audit_json(KRN_ABI_VERSION);
    let markdown = tooling_completeness_audit_markdown(KRN_ABI_VERSION);

    assert_eq!(TOOLING_COMPLETENESS_AUDIT, json);
    assert_eq!(TOOLING_COMPLETENESS_AUDIT_MARKDOWN, markdown);
    assert!(TOOLING_COMPLETENESS_AUDIT.contains("\"type\":\"tooling-completeness-audit\""));
    assert!(TOOLING_COMPLETENESS_AUDIT.contains("\"status\":\"repo-controlled-tools-complete\""));
    assert!(TOOLING_COMPLETENESS_AUDIT
        .contains("repository-controlled tooling and verification mechanisms only"));
    assert!(TOOLING_COMPLETENESS_AUDIT.contains("TOOL-UNIT-INTEGRATION"));
    assert!(TOOLING_COMPLETENESS_AUDIT.contains("TOOL-REPORT-WORKBENCH"));
    assert!(TOOLING_COMPLETENESS_AUDIT.contains("TOOL-ARTIFACT-IMPORT"));
    assert!(TOOLING_COMPLETENESS_AUDIT.contains("TOOL-TOOLING-COMPLETENESS"));
    assert!(TOOLING_COMPLETENESS_AUDIT.contains("docs/tooling_completeness_audit.json"));
    assert!(TOOLING_COMPLETENESS_AUDIT.contains("examples/krn_tooling_completeness_audit.rs"));
    assert!(TOOLING_COMPLETENESS_AUDIT.contains("external_evidence_still_required"));
    for gate in [
        "CERT-OPEN-001",
        "CERT-OPEN-009",
        "CERT-OPEN-010",
        ...
        assert!(TOOLING_COMPLETENESS_AUDIT.contains(gate));
    }
    assert!(TOOLING_COMPLETENESS_AUDIT_MARKDOWN.contains("# Hyperion Tooling Completeness Audit"));
    assert!(TOOLING_COMPLETENESS_AUDIT_MARKDOWN.contains("External Evidence Still Required"));
    assert!(PRELAB_CI_WORKFLOW.contains("Tooling completeness audit JSON drift"));
    assert!(PRELAB_CI_WORKFLOW.contains("Tooling completeness audit Markdown drift"));
    assert!(README.contains("krn_tooling_completeness_audit"));
    assert!(LAB_SUBMISSION_MANIFEST.contains("tooling completeness audit"));
}
...
fn prelab_quality_gates_are_reproducible_and_do_not_close_external_reports() {
    ...
    assert!(LAB_SUBMISSION_MANIFEST.contains(
        "formal coverage, integration, static-analysis, fuzzing, third-party assessment, and lab/tool reports remain pending"
    ));
...
#[test]
fn certification_report_workbench_is_reproducible_and_scoped() {
    let json = certification_report_pack_json(KRN_ABI_VERSION);
    let markdown = certification_report_markdown(KRN_ABI_VERSION);
    let html = certification_report_ui_html(KRN_ABI_VERSION);

    assert_eq!(CERTIFICATION_REPORT_PACK, json);
    assert_eq!(CERTIFICATION_REPORT_MARKDOWN, markdown);
    assert_eq!(CERTIFICATION_REPORT_UI, html);
    assert!(CERTIFICATION_REPORT_PACK.contains("\"type\":\"certification-report-pack\""));
    assert!(CERTIFICATION_REPORT_PACK.contains("docs/prelab_quality_gates.json"));
    ...
    assert!(CERTIFICATION_REPORT_PACK.contains("TIMEOUT-POLICY"));
    assert!(CERTIFICATION_REPORT_PACK.contains("krn_callback_timeout_policy"));
    assert!(CERTIFICATION_REPORT_PACK.contains("VARIABLE-DATA-BOUNDARY"));
    assert!(CERTIFICATION_REPORT_PACK.contains("krn_variable_data_boundary_audit"));
    ...
    assert!(README.contains("krn_certification_artifact_import"));
    ...
    assert!(README.contains("krn_basic_softpos"));
    assert!(README.contains("data-driven certification bundle"));
    assert!(README.contains("CDCVM capability signaling"));
    assert!(README.contains("krn_callback_timeout_policy"));
    assert!(README.contains("krn_variable_data_boundary_audit"));
    assert!(!CERTIFICATION_REPORT_PACK.contains("certified\":true"));
}

#[test]
fn certification_evidence_checklist_is_reproducible_and_scoped() {
    let json = certification_evidence_checklist_json(KRN_ABI_VERSION);
    let markdown = certification_evidence_checklist_markdown(KRN_ABI_VERSION);

    assert_eq!(CERTIFICATION_EVIDENCE_CHECKLIST, json);
    assert_eq!(CERTIFICATION_EVIDENCE_MARKDOWN, markdown);
    assert!(
        CERTIFICATION_EVIDENCE_CHECKLIST.contains("\"type\":\"certification-evidence-checklist\"")
    );
    assert!(CERTIFICATION_EVIDENCE_CHECKLIST.contains("\"checked_on\":\"2026-05-23\""));
    for required in [
        "CERT-OPEN-001",
        "CERT-OPEN-002",
        "CERT-OPEN-003",
        ...
        assert!(
            ...
            "evidence checklist missing {required}"
        );
    }
    assert!(CERTIFICATION_EVIDENCE_MARKDOWN.contains("# Hyperion Certification Evidence Checklist"));
    assert!(CERTIFICATION_EVIDENCE_MARKDOWN.contains("Every closure claim"));
    assert!(LAB_SUBMISSION_MANIFEST.contains("Certification evidence checklist"));
    assert!(LAB_SUBMISSION_MANIFEST.contains("Certification evidence intake ledger"));
    assert!(README.contains("Certification evidence checklist"));
    assert!(README.contains("certification_evidence_checklist.md"));
    assert!(README.contains("certification_evidence_intake.md"));
    assert!(TUTORIAL_README.contains("certification"));
    assert!(CERTIFICATION_OPEN_ISSUES.contains("Rows stay open"));
    assert!(!CERTIFICATION_EVIDENCE_CHECKLIST.contains("certified\":true"));
}

#[test]
fn certification_evidence_intake_ledger_is_reproducible_and_scoped() {
    let json = certification_evidence_intake_ledger_json(KRN_ABI_VERSION);
    let markdown = certification_evidence_intake_ledger_markdown(KRN_ABI_VERSION);

    assert_eq!(CERTIFICATION_EVIDENCE_INTAKE, json);
    assert_eq!(CERTIFICATION_EVIDENCE_INTAKE_MARKDOWN, markdown);
    assert!(
        CERTIFICATION_EVIDENCE_INTAKE.contains("\"type\":\"certification-evidence-intake-ledger\"")
    );
    for required in [
        "CERT-OPEN-001-ATTACHMENT",
        "CERT-OPEN-012-ATTACHMENT",
        "artifact_sha256",
        "submitted_build_scope",
        "supersedes",
        "SHA-256 required before review",
        "does_not_close",
        "pending external attachment",
    ] {
        assert!(
            CERTIFICATION_EVIDENCE_INTAKE.contains(required),
            "evidence intake ledger missing {required}"
        );
    }
    assert!(CERTIFICATION_EVIDENCE_INTAKE_MARKDOWN
        .contains("# Hyperion Certification Evidence Intake Ledger"));
    assert!(CERTIFICATION_EVIDENCE_INTAKE_MARKDOWN.contains("Superseded evidence"));
    assert!(LAB_SUBMISSION_MANIFEST.contains("Certification evidence intake ledger"));
    assert!(PRELAB_QUALITY_GATES.contains("PRELAB-CERTIFICATION-INTAKE-JSON"));
...
#[test]
fn certification_freeze_manifest_is_reproducible_and_scoped() {
    let json = certification_freeze_manifest_json(KRN_ABI_VERSION);
    let markdown = certification_freeze_manifest_markdown(KRN_ABI_VERSION);

    assert_eq!(CERTIFICATION_FREEZE_MANIFEST, json);
    assert_eq!(CERTIFICATION_FREEZE_MANIFEST_MARKDOWN, markdown);
    assert!(CERTIFICATION_FREEZE_MANIFEST
        .contains("\"type\":\"certification-freeze-manifest-template\""));
    for required in [
        "kernel_binary_hash",
        "config_bundle_hash",
        "capk_bundle_hash",
        ...
        "approval_package_hash",
        "SHA-256 pending",
        "pending external certification freeze",
        "CERT-OPEN-012",
        "does_not_close",
    ] {
        assert!(
            CERTIFICATION_FREEZE_MANIFEST.contains(required),
            "certification freeze manifest missing {required}"
        );
    }
    assert!(
        CERTIFICATION_FREEZE_MANIFEST_MARKDOWN.contains("# Hyperion Certification Freeze Manifest")
    );
    assert!(CERTIFICATION_FREEZE_MANIFEST_MARKDOWN.contains("SHA-256 pending"));
    assert!(LAB_SUBMISSION_MANIFEST.contains("Certification freeze manifest"));
    assert!(PRELAB_QUALITY_GATES.contains("PRELAB-CERTIFICATION-FREEZE-JSON"));
    assert!(PRELAB_QUALITY_GATES.contains("PRELAB-CERTIFICATION-FREEZE-MARKDOWN"));
    assert!(PRELAB_CI_WORKFLOW.contains("Certification freeze manifest JSON drift"));
    assert!(PRELAB_CI_WORKFLOW.contains("Certification freeze manifest Markdown drift"));
    assert!(README.contains("krn_certification_freeze_manifest"));
    assert!(!CERTIFICATION_FREEZE_MANIFEST.contains("\"certified\":true"));
}

#[test]
fn certification_security_assessment_plan_is_reproducible_and_scoped() {
    let json = certification_security_assessment_plan_json(KRN_ABI_VERSION);
    let markdown = certification_security_assessment_plan_markdown(KRN_ABI_VERSION);

    assert_eq!(CERTIFICATION_SECURITY_ASSESSMENT_PLAN, json);
    assert_eq!(CERTIFICATION_SECURITY_ASSESSMENT_PLAN_MARKDOWN, markdown);
    assert!(CERTIFICATION_SECURITY_ASSESSMENT_PLAN
        .contains("\"type\":\"certification-security-assessment-plan\""));
    for required in [
        "CERT-OPEN-008",
        "does_not_close",
        "source_commit",
        ...
        "profile_bundle_hash",
        "finding_disposition",
        "SEC-ASSESS-APDU-INJECTION",
        "SEC-ASSESS-FSM-BYPASS",
        "SEC-ASSESS-TRACE-LEAKAGE",
        "SEC-ASSESS-PROFILE-TAMPERING",
        "SEC-ASSESS-PIN-CUSTODY",
        "SEC-ASSESS-ODA-MATERIAL",
        "SEC-ASSESS-ISSUER-SCRIPTS",
        "SEC-ASSESS-REPORT-INTEGRITY",
        "traceability_foundation::krn_cert_004_penetration_rejects_apdu_injection_and_state_bypass",
        "accepted external assessment report",
    ] {
        assert!(
            CERTIFICATION_SECURITY_ASSESSMENT_PLAN.contains(required)
                || CERTIFICATION_SECURITY_ASSESSMENT_PLAN_MARKDOWN.contains(required),
            "security assessment plan missing {required}"
        );
    }
    assert!(CERTIFICATION_SECURITY_ASSESSMENT_PLAN_MARKDOWN
        .contains("# Hyperion Certification Security Assessment Plan"));
    assert!(LAB_SUBMISSION_MANIFEST.contains("Certification security assessment plan"));
    assert!(LAB_SUBMISSION_MANIFEST.contains("Third-party security assessment report"));
    assert!(PRELAB_QUALITY_GATES.contains("PRELAB-CERTIFICATION-SECURITY-JSON"));
    assert!(PRELAB_QUALITY_GATES.contains("PRELAB-CERTIFICATION-SECURITY-MARKDOWN"));
    assert!(PRELAB_CI_WORKFLOW.contains("Certification security assessment plan JSON drift"));
    assert!(PRELAB_CI_WORKFLOW.contains("Certification security assessment plan Markdown drift"));
    assert!(README.contains("krn_certification_security_assessment_plan"));
    assert!(!CERTIFICATION_SECURITY_ASSESSMENT_PLAN.contains("\"certified\":true"));
}

#[test]
fn certification_device_evidence_plan_is_reproducible_and_scoped() {
    let json = certification_device_evidence_plan_json(KRN_ABI_VERSION);
    let markdown = certification_device_evidence_plan_markdown(KRN_ABI_VERSION);

    assert_eq!(CERTIFICATION_DEVICE_EVIDENCE_PLAN, json);
    assert_eq!(CERTIFICATION_DEVICE_EVIDENCE_PLAN_MARKDOWN, markdown);
    assert!(CERTIFICATION_DEVICE_EVIDENCE_PLAN
        .contains("\"type\":\"certification-device-evidence-plan\""));
    for required in [
        "CERT-OPEN-005",
        "CERT-OPEN-006",
        "CERT-OPEN-007",
        ...
        assert!(
            CERTIFICATION_DEVICE_EVIDENCE_PLAN.contains(required)
                || CERTIFICATION_DEVICE_EVIDENCE_PLAN_MARKDOWN.contains(required),
            "device evidence plan missing {required}"
        );
    }
    assert!(CERTIFICATION_DEVICE_EVIDENCE_PLAN_MARKDOWN
        .contains("# Hyperion Device, L1, and PED Evidence Plan"));
    assert!(LAB_SUBMISSION_MANIFEST.contains("Certification device evidence plan"));
    assert!(LAB_SUBMISSION_MANIFEST.contains("Device and L1 evidence package"));
    assert!(PRELAB_QUALITY_GATES.contains("PRELAB-CERTIFICATION-DEVICE-JSON"));
    assert!(PRELAB_QUALITY_GATES.contains("PRELAB-CERTIFICATION-DEVICE-MARKDOWN"));
    assert!(PRELAB_CI_WORKFLOW.contains("Certification device evidence plan JSON drift"));
    assert!(PRELAB_CI_WORKFLOW.contains("Certification device evidence plan Markdown drift"));
    assert!(README.contains("krn_certification_device_evidence_plan"));
    assert!(!CERTIFICATION_DEVICE_EVIDENCE_PLAN.contains("\"certified\":true"));
}

#[test]
fn certification_integration_report_plan_is_reproducible_and_scoped() {
    let json = certification_integration_report_plan_json(KRN_ABI_VERSION);
    let markdown = certification_integration_report_plan_markdown(KRN_ABI_VERSION);

    assert_eq!(CERTIFICATION_INTEGRATION_REPORT_PLAN, json);
    assert_eq!(CERTIFICATION_INTEGRATION_REPORT_PLAN_MARKDOWN, markdown);
    assert!(CERTIFICATION_INTEGRATION_REPORT_PLAN
        .contains("\"type\":\"certification-integration-report-plan\""));
    for required in [
        "CERT-OPEN-009",
        "CERT-OPEN-012",
        "does_not_close",
        ...
        "INTEGRATION-L2-EXECUTION",
        "INTEGRATION-L3-ACQUIRER",
        "INTEGRATION-TRACE-COVERAGE",
        "INTEGRATION-OUTCOME-MAPPING",
        "INTEGRATION-DEVIATION-DISPOSITION",
        "INTEGRATION-BUILD-BINDING",
        "pending external",
    ] {
        assert!(
            CERTIFICATION_INTEGRATION_REPORT_PLAN.contains(required)
                || CERTIFICATION_INTEGRATION_REPORT_PLAN_MARKDOWN.contains(required),
            "integration report plan missing {required}"
        );
    }
    assert!(CERTIFICATION_INTEGRATION_REPORT_PLAN_MARKDOWN
        .contains("# Hyperion Integration Report Evidence Plan"));
    assert!(LAB_SUBMISSION_MANIFEST.contains("Certification integration report plan"));
    assert!(LAB_SUBMISSION_MANIFEST.contains("Integration test report"));
    assert!(PRELAB_QUALITY_GATES.contains("PRELAB-CERTIFICATION-INTEGRATION-JSON"));
    assert!(PRELAB_QUALITY_GATES.contains("PRELAB-CERTIFICATION-INTEGRATION-MARKDOWN"));
    assert!(PRELAB_CI_WORKFLOW.contains("Certification integration report plan JSON drift"));
    assert!(PRELAB_CI_WORKFLOW.contains("Certification integration report plan Markdown drift"));
    assert!(README.contains("krn_certification_integration_report_plan"));
    assert!(!CERTIFICATION_INTEGRATION_REPORT_PLAN.contains("\"certified\":true"));
}

#[test]
fn coverage_report_workflow_requires_100_percent_without_closing_cert_open_009() {
    assert!(COVERAGE_WORKFLOW.contains("100% Coverage Report Workflow"));
    assert!(COVERAGE_WORKFLOW.contains("CERT-OPEN-009"));
    assert!(COVERAGE_WORKFLOW.contains("scripts/coverage_100.sh"));
    assert!(COVERAGE_WORKFLOW.contains("krn_coverage_package_audit"));
    ...
    assert!(COVERAGE_AUDIT_EXAMPLE.contains("--require-package"));
    assert!(COVERAGE_AUDIT_EXAMPLE.contains("--require-certification-candidate"));
...
#[test]
fn coverage_package_audit_distinguishes_measurement_from_enforced_candidates() {
    let root = std::env::temp_dir().join(format!(
        "hyperion-traceability-coverage-audit-{}",
        std::process::id()
    ));
    if root.exists() {
        fs::remove_dir_all(&root).unwrap();
    ...
    let json = coverage_package_audit_json(KRN_ABI_VERSION, &audit);
    let markdown = coverage_package_audit_markdown(KRN_ABI_VERSION, &audit);
...
#[test]
fn prelab_ci_runs_quality_gates_and_coverage_without_certification_overclaim() {
    assert!(PRELAB_CI_WORKFLOW.contains("Pre-Lab Quality Gates"));
    assert!(PRELAB_CI_WORKFLOW.contains("cargo fmt --check"));
    assert!(PRELAB_CI_WORKFLOW.contains("cargo test"));
    ...
    assert!(PRELAB_CI_WORKFLOW.contains("coverage_audit.json"));
    ...
    assert!(PRELAB_CI_WORKFLOW.contains("krn_tooling_completeness_audit"));
    assert!(PRELAB_CI_WORKFLOW.contains("docs/tooling_completeness_audit.json"));
    assert!(PRELAB_CI_WORKFLOW.contains("docs/tooling_completeness_audit.md"));
    assert!(PRELAB_CI_WORKFLOW.contains("krn_certification_evidence_checklist"));
    assert!(PRELAB_CI_WORKFLOW.contains("docs/certification_evidence_checklist.json"));
    assert!(PRELAB_CI_WORKFLOW.contains("docs/certification_evidence_checklist.md"));
    assert!(PRELAB_CI_WORKFLOW.contains("krn_certification_evidence_intake"));
    assert!(PRELAB_CI_WORKFLOW.contains("docs/certification_evidence_intake.json"));
    assert!(PRELAB_CI_WORKFLOW.contains("docs/certification_evidence_intake.md"));
    assert!(PRELAB_CI_WORKFLOW.contains("Certification attachment audit smoke"));
    assert!(PRELAB_CI_WORKFLOW.contains("krn_certification_attachment_audit"));
    assert!(PRELAB_CI_WORKFLOW.contains("krn_certification_freeze_manifest"));
    assert!(PRELAB_CI_WORKFLOW.contains("docs/certification_freeze_manifest.json"));
    assert!(PRELAB_CI_WORKFLOW.contains("docs/certification_freeze_manifest.md"));
    assert!(PRELAB_CI_WORKFLOW.contains("krn_certification_security_assessment_plan"));
    assert!(PRELAB_CI_WORKFLOW.contains("docs/certification_security_assessment_plan.json"));
    assert!(PRELAB_CI_WORKFLOW.contains("docs/certification_security_assessment_plan.md"));
    assert!(PRELAB_CI_WORKFLOW.contains("krn_certification_device_evidence_plan"));
    assert!(PRELAB_CI_WORKFLOW.contains("docs/certification_device_evidence_plan.json"));
    assert!(PRELAB_CI_WORKFLOW.contains("docs/certification_device_evidence_plan.md"));
    assert!(PRELAB_CI_WORKFLOW.contains("krn_certification_integration_report_plan"));
    assert!(PRELAB_CI_WORKFLOW.contains("docs/certification_integration_report_plan.json"));
    assert!(PRELAB_CI_WORKFLOW.contains("docs/certification_integration_report_plan.md"));
    assert!(PRELAB_CI_WORKFLOW.contains("krn_certification_report_ui"));
    assert!(PRELAB_CI_WORKFLOW.contains("docs/certification_report_ui.html"));
    assert!(PRELAB_CI_WORKFLOW.contains("docs/certification_report_pack.json"));
    assert!(PRELAB_CI_WORKFLOW.contains("docs/certification_report_pack.md"));
    assert!(PRELAB_CI_WORKFLOW.contains("krn_certification_workspace"));
    assert!(PRELAB_CI_WORKFLOW.contains("target/hyperion-cert-workspace"));
    assert!(PRELAB_CI_WORKFLOW.contains("krn_basic_pos"));
    assert!(PRELAB_CI_WORKFLOW.contains("krn_basic_softpos"));
    assert!(PRELAB_CI_WORKFLOW.contains("krn_callback_timeout_policy"));
    assert!(PRELAB_CI_WORKFLOW.contains("Variable data boundary audit"));
    assert!(PRELAB_CI_WORKFLOW.contains("krn_variable_data_boundary_audit"));
...
#[test]
fn prelab_no_crash_smoke_is_reproducible_and_scoped() {
    let generated = prelab_no_crash_smoke_json().unwrap();

    assert_eq!(PRELAB_NO_CRASH_SMOKE, generated);
    assert!(PRELAB_NO_CRASH_SMOKE.contains("\"type\":\"prelab-no-crash-smoke\""));
    assert!(
    ...
    for case_id in [
        ...
        "ISSUER-SCRIPT-MALFORMED-COMMAND",
        "GAC-MISSING-MANDATORY-TAGS",
        "REPLAY-STRUCTURALLY-INVALID-COMMAND",
    ] {
        assert!(
            PRELAB_NO_CRASH_SMOKE.contains(case_id),
