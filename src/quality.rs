...
const QUALITY_GATES: &[QualityGate] = &[
    ...
    QualityGate {
        id: "PRELAB-TRACEPACK-AUDIT",
        command: "cargo run --quiet --example krn_trace_pack_audit -- --path docs/prelab_apdu_trace_pack.jsonl --require-prelab-fixture | diff -u docs/prelab_trace_pack_audit.json -",
        purpose: "regenerate and compare the masked pre-lab APDU trace-pack audit",
    },
    QualityGate {
        id: "PRELAB-TRACEPACK-AUDIT-MARKDOWN",
        command: "cargo run --quiet --example krn_trace_pack_audit -- --path docs/prelab_apdu_trace_pack.jsonl --markdown | diff -u docs/prelab_trace_pack_audit.md -",
        purpose: "regenerate and compare the reviewable masked pre-lab APDU trace-pack audit report",
    },
    QualityGate {
    ...
    QualityGate {
        id: "PRELAB-TOOLING-COMPLETENESS-JSON",
        command: "cargo run --quiet --example krn_tooling_completeness_audit -- --json | diff -u docs/tooling_completeness_audit.json -",
        purpose: "regenerate and compare the repository-controlled tooling completeness audit JSON",
    },
    QualityGate {
        id: "PRELAB-TOOLING-COMPLETENESS-MARKDOWN",
        command: "cargo run --quiet --example krn_tooling_completeness_audit -- --markdown | diff -u docs/tooling_completeness_audit.md -",
        purpose: "regenerate and compare the reviewable repository-controlled tooling completeness audit report",
    },
    QualityGate {
    ...
    QualityGate {
        id: "PRELAB-CERTIFICATION-ATTACHMENT-AUDIT",
        command: "cargo run --quiet --example krn_certification_attachment_audit -- --root target/hyperion-cert-attachments",
        purpose: "smoke-test the certification attachment audit hash inventory tool",
    },
    QualityGate {
        id: "PRELAB-CERTIFICATION-WIZARD",
        command: "cargo run --quiet --example krn_certification_wizard -- --non-interactive --out target/hyperion-certification-wizard",
        purpose: "smoke-test the guided certification workspace and data-bundle wizard",
    },
    QualityGate {
        id: "PRELAB-CERTIFICATION-ARTIFACT-IMPORT-JSON",
        command: "cargo run --quiet --example krn_certification_artifact_import -- --plan-json | diff -u docs/certification_artifact_import_plan.json -",
        purpose: "regenerate and compare the certification artifact import adapter plan JSON",
    },
    QualityGate {
        id: "PRELAB-CERTIFICATION-ARTIFACT-IMPORT-MARKDOWN",
        command: "cargo run --quiet --example krn_certification_artifact_import -- --plan-markdown | diff -u docs/certification_artifact_import_plan.md -",
        purpose: "regenerate and compare the reviewable certification artifact import adapter plan",
    },
    QualityGate {
        id: "PRELAB-CERTIFICATION-ARTIFACT-IMPORT-SMOKE",
    ...
    QualityGate {
        id: "PRELAB-CERTIFICATION-WORKSPACE",
        command: "cargo run --quiet --example krn_certification_workspace -- --out target/hyperion-cert-workspace",
        purpose: "generate the complete local certification report workspace",
    },
    QualityGate {
        id: "PRELAB-BASIC-POS",
    ...
    QualityGate {
        id: "PRELAB-BASIC-SOFTPOS",
        command: "cargo run --quiet --example krn_basic_softpos",
        purpose: "run the basic mobile NFC SoftPoS integration example through data-driven contactless online approval",
    },
    QualityGate {
        id: "PRELAB-CALLBACK-TIMEOUT-POLICY",
        command: "cargo run --quiet --example krn_callback_timeout_policy",
        purpose: "emit the ABI callback timeout policy for adapter startup evidence",
    },
    QualityGate {
        id: "PRELAB-VARIABLE-DATA-BOUNDARY",
        command: "cargo run --quiet --example krn_variable_data_boundary_audit -- src",
        purpose: "audit production Rust source for compiled scheme, CAPK, TAC/IAC, and profile fixture values",
    },
    QualityGate {
