...
impl Tvr {
    ...
    pub fn set(&mut self, bit: (usize, u8)) {
        let Some(byte) = self.0.get_mut(bit.0) else {
            return;
        };
        let allowed = Self::ALLOWED_MASKS[bit.0];
        *byte |= bit.1 & allowed;
    }
    ...
    pub fn is_set(self, bit: (usize, u8)) -> bool {
        self.0.get(bit.0).is_some_and(|byte| byte & bit.1 != 0)
    }
...
impl Tsi {
    pub const ALLOWED_MASKS: [u8; 2] = [0xfc, 0x00];
...
mod tests {
    ...
    fn tvr_and_tsi_mutation_masks_rfu_bits() {
        ...
        tsi.set((0, 0x03));
        tsi.set((1, 0xff));
        assert_eq!(tsi.bytes(), [0, 0]);
        assert!(!tsi.has_rfu_bits());

        tsi.set(Tsi::ISSUER_AUTHENTICATION_PERFORMED);
